...
plot.groups = c("Group","CellLine")
plot.types = c("discrete","discrete")
...
samples.percent.detected = 100 *apply(percent.mat, 2, count.defined.values) / nrow(percent.mat)
probes.percent.detected = 100*apply(percent.mat, 1, count.defined.values) / ncol(percent.mat)
print(dim(percent.mat))
percent.mat = percent.mat[probes.percent.detected == 100,]
print(dim(percent.mat))
percent.detected = data.frame(Sample = userID, Percent.Detected=samples.percent.detected)
write.table(percent.detected, paste(output.prefix,"_percent_detected.txt",sep=""),
				sep="\t", quote=F, row.names=F)

quantiles = round(apply(percent.mat, 2, quantile, na.rm=TRUE, probs=c(0.01,0.05,0.25,0.5,0.75,0.95,0.99)), digits=2)
quantiles = data.frame(Percent = rownames(quantiles), quantiles)
write.table(quantiles, paste(output.prefix,"_percent_quantiles.txt",sep=""),
				sep="\t", quote=F, row.names=F)
...
for (i in 1:length(plot.groups)){
	...
	print(group)
	temp.mat = percent.mat[,!is.na(sample.table[,group])]
	#print(dim(temp.mat))
	...
	pca.text.file = paste(output.prefix,"_",group,"_pca_values.txt",sep="")
	...
	pca.file = paste(output.prefix,"_pca_by_",group,".png",sep="")
	png(file=pca.file)
	if(group.type == "continuous"){
		...
		plot(pc.values$PC1, pc.values$PC2, col = labelColors, xlab = paste("PC1 (",round(100* variance.explained[1] , digits = 2),"%)", sep = ""),
				ylab = paste("PC2 (",round(100* variance.explained[2] , digits = 2),"%)", sep = ""),
				pch=19, main=paste("Color by ",group,sep=""))
		legend("right",legend=groups,col=color.palette, inset=-0.5, xpd=T, pch=19, cex=0.7)
	}
	dev.off()

	box.file = paste(output.prefix,"_box_plot_by_",group,".png",sep="")
	png(file=box.file)
	if(group.type == "continuous"){
	...
	cluster.file = paste(output.prefix,"_cluster_by_",group,".png",sep="")
	...
	cluster.order.file = paste(output.prefix,"_cluster_by_",group,".txt",sep="")
	...
	hist.file = paste(output.prefix,"_density_by_",group,".png",sep="")
	png(file = hist.file)
	par(mar = par("mar") + c(0,0,0,10)) 
	for (j in 1:ncol(temp.mat))
		{		
			...
			if(j == 1)
				{
					den = density(data, na.rm=T,from=0, to=100)
					expr = den$x
					freq = den$y
					plot(expr, freq, type="l", xlab = paste("Percent Methylation",sep=""), ylab = "Density",
							xlim=c(0,100), ylim=c(0,density.max), col=labelColors[j], main=paste("Color by ",group,sep=""))
					if(group.type == "continuous"){
						legend("right",legend=c(round(plot.var.max,digits=1),rep("",length(color.range)-2),round(plot.var.min,digits=1)),
								col=rev(color.range),  pch=15, inset=-0.4, xpd=T, y.intersp = 0.4, cex=0.8, pt.cex=1.5)
					}else{
						legend("right",legend=groups,col=color.palette,  pch=19, inset=-0.5, xpd=T,  cex=0.7)
					}
				}#end if(j == 1)
			else
				{
