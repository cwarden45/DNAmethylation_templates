...
write.table(quantiles, paste(beta.prefix,beta.normalization,"beta_quantiles.txt",sep="_"),sep="\t", quote=F, row.names=F)
...
for (i in 1:length(plot.groups)){
	...
	pca.text.file = paste(beta.prefix,beta.normalization,group,"pca_values.txt",sep="_")
	...
	pca.file = paste(beta.prefix,"_",beta.normalization,"_pca_by_",group,".png",sep="")
	png(file=pca.file)
	if(group.type == "continuous"){
	...
	box.file = paste(beta.prefix,"_",beta.normalization,"_box_plot_by_",group,".png",sep="")
	png(file=box.file)
	if(group.type == "continuous"){
		#par(mar = par("mar") + c(0,0,0,5))
	...
	cluster.file = paste(beta.prefix,"_",beta.normalization,"_cluster_by_",group,".png",sep="")
	...
	cluster.order.file = paste(beta.prefix,"_",beta.normalization,"_cluster_by_",group,".txt",sep="")
	...
	hist.file = paste(beta.prefix,"_",beta.normalization,"_density_by_",group,".png",sep="")
	png(file = hist.file)
	par(mar = par("mar") + c(0,0,0,10)) 
